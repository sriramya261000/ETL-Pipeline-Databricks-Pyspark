...
customers_df=(spark.read.format("delta").table("bronze_catalog.customers_db.customers_raw")
...
(customers_df.write.format("delta") 
        .mode("append") 
        .saveAsTable("silver_catalog.customers_db.customers_cleaned"))
...
(invoices_df.write.format("delta") 
        .mode("append") 
        .saveAsTable("silver_catalog.invoices_db.invoices_cleaned"))
...
if "customers_scd" not in tables_list:
    df = (spark.read.table("silver_catalog.customers_db.customers_cleaned")
                .dropDuplicates()
    ...
    sourceDF = spark.read.table("silver_catalog.customers_db.customers_cleaned")
    # display(sourceDF)
